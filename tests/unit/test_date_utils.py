"""Unit tests for date parsing and utility functions in routes/activities.py."""
...
class TestParseIso:
    ...
    def test_invalid_format(self):
        ...
        assert _parse_iso("03-15-2026") is None
        assert _parse_iso("not-a-date") is None
        assert _parse_iso("2026/03/15") is None

    def test_truncates_to_ten_chars(self):
        """Datetime strings are truncated to date portion."""
        result = _parse_iso("2026-03-15T10:30:00")
        assert result == datetime.date(2026, 3, 15)

    def test_invalid_date_values(self):
        """Invalid date values (e.g., month 13) return None."""
        assert _parse_iso("2026-13-15") is None
        assert _parse_iso("2026-02-30") is None


class TestParseWeekdays:
    """Tests for the _parse_weekdays function."""

    def test_single_day(self):
        """Parse a single weekday."""
        assert _parse_weekdays("Mon") == [0]
        assert _parse_weekdays("Fri") == [4]
        assert _parse_weekdays("Sun") == [6]

    def test_multiple_days(self):
        """Parse multiple weekdays separated by commas."""
        result = _parse_weekdays("Mon, Wed, Fri")
        assert result == [0, 2, 4]

    def test_all_days(self):
        """Parse all weekdays."""
        result = _parse_weekdays("Mon, Tue, Wed, Thu, Fri, Sat, Sun")
    ...
    def test_full_day_names(self):
        ...
        result = _parse_weekdays("Monday, Wednesday")
        assert result == [0, 2]
    ...
    def test_invalid_days(self):
        ...
        result = _parse_weekdays("Mon, Invalid, Wed")
        assert result == [0, 2]

    def test_extra_whitespace(self):
        """Extra whitespace is handled."""
        result = _parse_weekdays("  Mon  ,  Wed  ")
        assert result == [0, 2]


class TestExpandPatternDates:
    """Tests for the _expand_pattern_dates function."""
    ...
    def test_single_week_pattern(self):
        ...
        result = _expand_pattern_dates(pattern)
        expected = {
    ...
    def test_missing_start_date(self):
        ...
        assert _expand_pattern_dates(pattern) == set()
